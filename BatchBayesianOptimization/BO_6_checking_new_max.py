...
for seed in range(10):   # 0..9
    ...
    run_best = max(BO_run.Y)
    print(f"Seed {seed}: best = {run_best}")
    if run_best > best_overall_y:
        best_overall_y = run_best
        best_overall_x = BO_run.X_lab[np.argmax(BO_run.Y)]

print("Best over all seeds:", best_overall_y)
print("Best X:", best_overall_x)
