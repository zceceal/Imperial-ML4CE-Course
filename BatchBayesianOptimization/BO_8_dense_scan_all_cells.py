...
class BO:
    def __init__(self,
                 max_iters=10,   # already enough to reach the max in your previous runs
                 batch_size=5,
                 n_init=6,
                 n_cand=300,
                 multi_hyper=1,
    ...
    def _propose_batch(self, n_cand, batch_size):
        ...
        explore_prob = 0.1  # 10% of the time, ignore EI and explore
...
best_T_ct2   = 32.347412109375
best_pH_ct2  = 6.847412109375
best_F1_ct2  = 35.028076171875
best_F2_ct2  = 31.585693359375
best_F3_ct2  = 27.825927734375

best_T_ct3   = 33.4100341796875
best_pH_ct3  = 6.3555908203125
best_F1_ct3  = 36.1541748046875
best_F2_ct3  = 44.5831298828125
best_F3_ct3  = 16.3177490234375

X_ct2 = [best_T_ct2, best_pH_ct2, best_F1_ct2, best_F2_ct2, best_F3_ct2, 'celltype_2']
X_ct3 = [best_T_ct3, best_pH_ct3, best_F1_ct3, best_F2_ct3, best_F3_ct3, 'celltype_3']

Y_ct2 = virtual_lab.conduct_experiment([X_ct2])[0]
Y_ct3 = virtual_lab.conduct_experiment([X_ct3])[0]

print("Titre at best continuous point with celltype_2:", Y_ct2)
print("Titre at best continuous point with celltype_3:", Y_ct3)

# -----------------------------------------------------
# DENSE SOBOL SCAN OVER ALL CELL TYPES
# -----------------------------------------------------

N_scan = 10000  # you can increase later if it's fast enough

cell_types = ['celltype_1', 'celltype_2', 'celltype_3']

best_overall_titre = -np.inf
best_overall_X = None
best_overall_celltype = None

for ct in cell_types:
    print(f"\n--- Dense Sobol scan for {ct} ---")
    ...
    cell_types_scan = np.array([ct] * N_scan)
    ...
    print("Best titre for", ct, ":", best_titre_scan)
    print("Best X for", ct, ":", best_X_scan)

    if best_titre_scan > best_overall_titre:
        best_overall_titre = best_titre_scan
        best_overall_X = best_X_scan
        best_overall_celltype = ct

print("\n===== SUMMARY OVER ALL CELL TYPES =====")
print("Best overall titre:", best_overall_titre)
print("Best overall X:", best_overall_X)
print("Best overall cell type:", best_overall_celltype)

print("\n===== COMPARISON WITH CURRENT BO OPTIMUM (celltype_3 only) =====")
print("BO best titre (celltype_3):", max(BO_m.Y))
print("BO best X (celltype_3):", BO_m.X_lab[np.argmax(BO_m.Y)])
