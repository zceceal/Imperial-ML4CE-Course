...
# -----------------------------------------------------
best_overall_y = -np.inf
best_overall_x = None
best_seed = None

for seed in [0, 1, 2, 3, 4]:  # 5 runs with different random starts
    print(f"\n=== Running BO with seed={seed} ===")
    BO_run = BO(
        max_iters=40,
        batch_size=5,
        n_init=6,
        ...
        multi_hyper=1,
        seed=seed,
        time_budget=120,   # or None if you don’t care about the “score” metric
    )

    this_best_y = max(BO_run.Y)
    this_best_x = BO_run.X_lab[np.argmax(BO_run.Y)]

    print(f"  Run best titre: {this_best_y}")
    print(f"  Run best X: {this_best_x}")

    if this_best_y > best_overall_y:
        best_overall_y = this_best_y
        best_overall_x = this_best_x
        best_seed = seed

print("\n===== SUMMARY ACROSS RUNS =====")
print("Best titre over all seeds:", best_overall_y)
print("Best X over all seeds:", best_overall_x)
print("Seed that achieved it:", best_seed)