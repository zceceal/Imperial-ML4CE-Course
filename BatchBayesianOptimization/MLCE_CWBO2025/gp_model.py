...
class GP_model:
    ...
    def GP_inference_np(self, x):
        ...
        for i in range(ny_dim):
            ...
            val_mean = np.matmul(np.matmul(k.T, invK), Ysample[:, i])
            mean[i]  = float(np.asarray(val_mean).squeeze())
            val_var  = sf2opt - np.matmul(np.matmul(k.T, invK), k)
            val_var  = float(np.asarray(val_var).squeeze())
            var[i]   = max(0.0, val_var)  # numerical safeguard
